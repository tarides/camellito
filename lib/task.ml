...
1. Code samples from the tutorials can be executed in the provided environment.
   ...
   expression and press [Return]. The output in the Utop should be the same as in
   the tutorial. You can also enter your expressions, try [2 + 2] for instance
   (always finish your input by [;;] and [Return]). Exit [utop] with [Ctrl+D] or
   [#quit;;]
